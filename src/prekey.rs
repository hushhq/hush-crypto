...
/// One entry for one-time pre-key upload.
...
pub fn generate_signed_pre_key(
    ...
    Ok((public_33, out.signature.to_vec()))
}

/// Generates n one-time pre-keys (public part only; private keys are ephemeral in X3DH).
